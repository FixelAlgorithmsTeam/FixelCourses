...
if __name__ == '__main__':
    ...
    entityName  = dEnv[WANDB_ENTITY]  #<! Extract the Entity Name
    projName    = dEnv[WANDB_PROJECT_NAME] #<! Extract the Project Name
    sweepId     = dEnv[WANDB_SWEEP_ID]     #<! Extract the Sweep ID
    ...
    # Generated in `0005TrainYolo.py`
    print(f'Sweep ID: {sweepId}') #<! Print the Sweep ID to use it later

    wandb.agent(sweepId, function = hTrainYoloModel, entity = entityName, project = projName, count = numExp) #>! count = number of runs to perform
