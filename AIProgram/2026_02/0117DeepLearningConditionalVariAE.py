...
tuImgSize = (28, 28)       #<! Image spatial size
...
class ConditionalVariationalAutoEncoder(nn.Module):
    def __init__( self, numCls: int, tuImgSize: Tuple[int, int], latDim: int, decEmbedDim: int ) -> None:
        ...
        self.oEncEmbedding = nn.Embedding(num_embeddings = numCls, embedding_dim = tuImgSize[0] * tuImgSize[1])
        ...
        self.oDecEmbedding = nn.Embedding(num_embeddings = numCls, embedding_dim = decEmbedDim)
        self.oDec = nn.Sequential(
            nn.Unflatten(dim = 1, unflattened_size = (decDim, 1, 1)),
    ...
    def forward( self, tX: Union[Tensor, ConditionalInput], vY: Optional[Tensor] = None ) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        tX = torch.cat((tX, self.oEncEmbedding(vY).view(-1, 1, *self._tuImgSize)), dim = 1)
        ...
        mZ = torch.cat((mZ, self.oDecEmbedding(vY)), dim = 1)
        tXHat = self.oDec(mZ)
...
def Main( csvFileName: str, csvFileUrl: str, tuImgSize: Tuple[int, int], numCls: int, decEmbedDim: int, latDim: int, recLossType: Literal['MAE', 'MSE'], β: float, batchSize: int, numWorkers: int, numEpochs: int ) -> None:
    ...
    dlTrain = torch.utils.data.DataLoader(ConditionalDataset(dsTrain), batch_size = batchSize, shuffle = True, num_workers = numWorkers, pin_memory = torch.cuda.is_available(), drop_last = True, persistent_workers = persistentWorkers)
    dlVal   = torch.utils.data.DataLoader(ConditionalDataset(dsVal), shuffle = False, batch_size = 2 * batchSize, num_workers = numWorkers, pin_memory = torch.cuda.is_available(), drop_last = False, persistent_workers = persistentWorkers)
...
if __name__ == '__main__':
    Main(csvFileName, csvFileUrl, tuImgSize, numCls, decEmbedDim, latDim, recLossType, β, batchSize, numWorkers, numEpochs)